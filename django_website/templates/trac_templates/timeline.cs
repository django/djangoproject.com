...
<div id="content" class="timeline">
 ...
 <form id="prefs" method="get" action="<?cs var:trac.href.timeline ?>">
  ...
  <fieldset><?cs
   each:filter = timeline.filters ?><div class="field">
     <input type="checkbox" id="<?cs var:filter.name ?>" name="<?cs var:filter.name ?>"<?cs
      if:filter.enabled ?> checked="checked"<?cs /if ?> /> 
      <label for="<?cs var:filter.name ?>"><?cs var:filter.label ?></label></div><?cs
   /each ?>
  </fieldset>
  <div class="buttons">
