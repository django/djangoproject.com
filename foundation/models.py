...
class CoreAwardCohort(models.Model):
    """
    A cohort of individuals -- such as "Q1 2021" -- receiving the
    ...
    name = models.CharField(
        max_length=255,
        unique=True,
        help_text="Name for the group being inducted -- for example, 'Q1 2021'",
    )
...
class CoreAward(models.Model):
    ...
    cohort = models.ForeignKey(
        CoreAwardCohort, related_name="recipients", on_delete=models.CASCADE
    )
    recipient = models.TextField(help_text="Recipient's name")
    link = models.URLField(
        blank=True, null=True, help_text="Optional link for this recipient"
    )
    description = models.TextField(
        blank=True,
        help_text="Optional one-paragraph description/bio of why this person received the award",
    )
