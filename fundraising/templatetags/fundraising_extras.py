...
from members.models import CORPORATE_MEMBERSHIP_AMOUNTS, CorporateMember, Invoice
...
@register.inclusion_tag(
    "fundraising/includes/donation_form_with_heart.html", takes_context=True
)
def donation_form_with_heart(context):
    ...
    donated_amount += (
        Invoice.objects.filter(paid_date__gte=GOAL_START_DATE).aggregate(
            models.Sum("amount")
        )["amount__sum"]
        or 0
    )
