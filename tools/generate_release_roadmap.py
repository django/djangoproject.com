...
def load_release_data(json_file):
    ...
    processed_data = []
    for item in data:
        item['release_date'] = dtime.datetime.strptime(item['release_date'], '%Y-%m-%d').date()
        item['mainstream_end'] = dtime.datetime.strptime(item['mainstream_end'], '%Y-%m-%d').date()
        item['extended_end'] = dtime.datetime.strptime(item['extended_end'], '%Y-%m-%d').date()
    ...
    for i, year in enumerate(range(start_year, end_year + 1)):
        ...
        for i in range(3):
            grid_lines.append({
                ...
                "width": config['year_line_width'] if i==0 else config['month_line_width'],
...
def generate_releases(data, start_year, config):
    ...
    for i, release in enumerate(data):
        bar_y = config['padding_top'] + (i * (config['bar_height'] + config['bar_v_spacing']))
